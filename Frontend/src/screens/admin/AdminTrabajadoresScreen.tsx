...
const AdminTrabajadoresScreen: React.FC = () => {
  ...
  // 🎯 NUEVO: Obtenemos el usuario actual de la sesión
  const { user } = useAuth(); 
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        const lista = res.data
          ...
          // 🎯 NUEVO: Filtramos a los clientes Y también al usuario actual validando su email
  ...
  useEffect(() => { cargarTrabajadores(); }, []); // 🎯 Como user no debería cambiar de la nada, no es necesario pasarlo al arreglo de dependencias aquí
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean, nombre: string) => {
    ...
    if (!window.confirm(`¿Estás seguro de que deseas ${accionTexto} a ${nombre}?`)) {
      return;
    }
  ...
  /* ── roles únicos dinámicos desde BD ── */
    ...
    <div className="at-wrap animate-in">
      ...
      <div className="at-stats">
        {[
          ...
          <div className="at-stat" key={s.label}>
            <strong>{s.n}</strong>
            <span>{s.label}</span>
          </div>
        ))}
      ...
      <div className="at-toolbar">
        <div className="at-search-box">
          ...
          />
          {query && (
            <button className="at-search-clear" onClick={() => setQuery('')}>✕</button>
          )}
        </div>
        ...
        {loading ? (
          <div className="at-state">
            <div className="at-spinner">
              <div className="at-ring"/><div className="at-ring at-ring--2"/>
            </div>
          ...
          <div className="at-state">
            ...
            <p>{query ? 'Sin resultados para tu búsqueda' : 'No hay personal registrado aún (o solo estás tú)'}</p>
            {query
              ? <button className="at-btn-retry" onClick={() => setQuery('')}>Limpiar búsqueda</button>
              : <button className="at-btn-retry" onClick={() => navigate('/admin-trabajadores/nuevo')}>Registrar primero</button>
            }
          </div>
          ...
          <table className="at-table">
            ...
            <tbody>
              {lista.map((t, i) => (
                <tr
                  key={t.id}
                  className={`at-row ${!t.activo ? 'at-row--inactive' : ''}`}
                ...
                >
                  ...
                  <td>
                    <div className="at-employee">
                      <div
                        className="at-avatar"
                        style={{ 
                          '--ac': t.activo ? generarColorDinamico(t.rol) : '#6b7280',
                      ...
                      <div className="at-employee-info">
                        ...
                        {t.fecha_ingreso && (
                          <small>
                            Desde {new Date(t.fecha_ingreso).toLocaleDateString('es-MX', {
                  ...
                  <td>
                    <span
                      className="at-role-badge"
                      style={{ 
                        '--rc': t.activo ? generarColorDinamico(t.rol) : '#6b7280',
                  ...
                  {/* ── ACCIONES ── */}
                  <td className="at-actions-cell" style={{ display: 'flex', gap: '8px' }}>
                    {/* Botón Editar */}
                    <button
                      className="at-icon-btn at-icon-btn--edit"
                      title="Editar trabajador"
                      onClick={() => navigate(`/admin-trabajadores/editar/${t.id}`)}
                    >
                    ...
                    <button
                      ...
                      title={t.activo ? "Suspender trabajador" : "Reactivar trabajador"}
                      onClick={() => handleToggleStatus(t.id, t.activo, t.nombre)}
                      style={{ 
                        color: t.activo ? '#ef4444' : '#10b981', 
                        backgroundColor: t.activo ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)' 
                      }}
                    >
                      {t.activo ? (
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="15" height="15">
