...
const AdminPageContentInitialScreen: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // 1. Obtener todas las páginas y filtrar por slug 'inicio'
      const paginas = await paginasAPI.getAll();
      const paginaInicio = Array.isArray(paginas)
        ? paginas.find((p: Pagina) => p.slug === 'inicio')
        : paginas.data?.find((p: Pagina) => p.slug === 'inicio');

      if (!paginaInicio) {
        setError('Página "Inicio" no encontrada');
        setLoading(false);
        return;
      }

      setPagina(paginaInicio);

      // 2. Obtener secciones
      const seccionesData = await seccionesAPI.getByPagina(paginaInicio.id);
  ...
  const handleSaveContent = async () => {
    ...
    try {
      if (editingContentId) {
        ...
        setSuccessMessage('Contenido agregado correctamente');
      }
  ...
  const handleHideContent = async (contenidoId: number) => {
    try {
      ...
      setSuccessMessage('Contenido ocultado correctamente');
      fetchInitialData();
    } catch (err) {
      console.error('Error ocultando contenido:', err);
      setError('Error al ocultar el contenido');
    }
  };
  ...
  const handleDeleteContent = async (contenidoId: number) => {
    ...
    try {
      ...
      setSuccessMessage('Contenido eliminado correctamente');
      fetchInitialData();
    } catch (err) {
  ...
  return (
    <div className="initial-page-content-container">
      {/* MENSAJES */}
      ...
      {/* MODAL - FORMULARIO DE CONTENIDO */}
      {showContentForm && (
        <div className="modal-overlay" onClick={handleCloseContentForm}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{editingContentId ? 'Editar Contenido' : 'Agregar Contenido'}</h2>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <label htmlFor="enlace_url">URL del Enlace</label>
                <input
                  type="url"
                  id="enlace_url"
                  name="enlace_url"
            ...
            <div className="modal-footer">
              ...
              <button className="btn-save" onClick={handleSaveContent}>
                {editingContentId ? 'Actualizar' : 'Agregar'} Contenido
              </button>
            </div>
          </div>
        </div>
